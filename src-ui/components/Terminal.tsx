...
export default function Terminal({workspace}:TerminalProps){
  ...
  const inputBuffers=useRef<Map<string,string>>(new Map()),inputTimers=useRef<Map<string,number>>(new Map());
  ...
  const openingSessions=useRef<Set<string>>(new Set());
  activeIdRef.current=activeTerminalId;
  const updateTerminal=async(id:string,changes:Partial<ControlTerminal>)=>{try{const collection=getControlDatabase().terminals;if(await collection.exists(id))await collection.update(id,changes);}catch(reason){const message=reason instanceof Error?reason.message:String(reason);if(!/not found|cancel/i.test(message))throw reason;}};
  ...
  useEffect(()=>{
    if(!activeTerminal||sessionPids.has(activeTerminal.id)||openingSessions.current.has(activeTerminal.id))return;
    openingSessions.current.add(activeTerminal.id);
    void invoke<number>("cmd_terminal_session_open",{sessionId:activeTerminal.id,cwd:activeTerminal.cwd}).then(response=>{
      if(response.success&&response.data){
        setSessionPids(current=>new Map(current).set(activeTerminal.id,response.data!));
        readySessions.current.add(activeTerminal.id);
        for(const resolve of readyWaiters.current.get(activeTerminal.id)||[])resolve();
        readyWaiters.current.delete(activeTerminal.id);
        if(activeIdRef.current===activeTerminal.id&&xtermRef.current){xtermRef.current.options.disableStdin=false;xtermRef.current.focus();}
        setError(undefined);
      }
      else setError(response.error||"Could not open terminal session");
    }).finally(()=>openingSessions.current.delete(activeTerminal.id));
  ...
  const queueInput=(sessionId:string,input:string)=>{
    inputBuffers.current.set(sessionId,(inputBuffers.current.get(sessionId)||"")+input);
    if(inputTimers.current.has(sessionId))return;
    inputTimers.current.set(sessionId,window.setTimeout(()=>{
      inputTimers.current.delete(sessionId);
      const buffered=inputBuffers.current.get(sessionId)||"";inputBuffers.current.delete(sessionId);
      if(buffered)queueWrite(sessionId,buffered);
    },8));
  };
    ...
    void listen<TerminalEvent>("terminal-event",({payload})=>{
      ...
      const next=prior.catch(()=>undefined).then(async()=>{
        if(payload.eventType==="output"&&payload.output){
          ...
          const record=await getControlDatabase().terminals.get(payload.sessionId);if(record)await updateTerminal(record.id,{history:compact([...record.history,...cleanTerminalOutput(ready)]).slice(-5000),updatedAt:Date.now()});
        }
        if(payload.eventType==="closed"){
          ...
          if(tail){const record=await getControlDatabase().terminals.get(payload.sessionId);if(record)await updateTerminal(record.id,{history:compact([...record.history,...cleanTerminalOutput(tail)]).slice(-5000),updatedAt:Date.now()});}
  ...
  useEffect(()=>{
    ...
    const send=terminal.onData(data=>{queueInput(session.id,data);if(data.includes("\r"))window.setTimeout(()=>void invoke<string>("cmd_terminal_session_cwd",{sessionId:session.id}).then(async response=>{if(response.success&&response.data){const record=await getControlDatabase().terminals.get(session.id);if(record&&record.cwd!==response.data)await updateTerminal(session.id,{cwd:response.data!,updatedAt:Date.now()});}}),100);});
    ...
    const fitAndSync=()=>{try{fit.fit();void invoke("cmd_terminal_session_resize",{sessionId:session.id,rows:terminal.rows,cols:terminal.cols});}catch{/* terminal may be disposing */}};
    const observer=new ResizeObserver(fitAndSync);observer.observe(host);requestAnimationFrame(()=>{fitAndSync();terminal.focus();});
  ...
  const closeTerminal=async(id:string)=>{await invoke("cmd_terminal_session_close",{sessionId:id});const readinessTimer=readinessTimers.current.get(id);if(readinessTimer)window.clearTimeout(readinessTimer);const inputTimer=inputTimers.current.get(id);if(inputTimer)window.clearTimeout(inputTimer);inputTimers.current.delete(id);inputBuffers.current.delete(id);readinessTimers.current.delete(id);pendingOutput.current.delete(id);writeQueues.current.delete(id);readySessions.current.delete(id);readyWaiters.current.delete(id);await getControlDatabase().terminals.delete(id);};
  useEffect(()=>{const runTests=async()=>{if(!workspace)return setError("Open a project before running tests");setError(undefined);const response=await invoke<string>("cmd_project_test_command",{workspacePath:workspace.path});if(!response.success||!response.data)return setError(response.error||"No test command detected");const sessionId=activeTerminalId||await createTerminal("test");if(!sessionId)return setError("Could not create a test terminal");queueWrite(sessionId,`${response.data}\r`);};const runOperational=async(detail:{command:string;commandId?:string;title?:string})=>{if(!workspace)return setError("Open a project before running a repository command");const db=getControlDatabase(),now=Date.now(),workId=createId("work"),runId=createId("run");await db.works.insert({id:workId,projectId:workspace.projectId,title:detail.title||detail.command,intent:"Execute a verified repository operation",kind:"maintenance",status:"active",createdAt:now,updatedAt:now},workId);await db.runs.insert({id:runId,projectId:workspace.projectId,workId,agentId:"local-user",intelligenceId:"local-user",skillIds:[],capabilityIds:["shell"],runtime:"terminal",status:"running",heartbeatAt:now,startedAt:now},runId);if(detail.commandId)await db.operationalCommands.update(detail.commandId,{runId,lastVerifiedAt:now,verifiedBy:"local-user",lastResult:"unknown",updatedAt:now});const sessionId=activeTerminalId||await createTerminal("test");if(!sessionId)return setError("Could not create a command terminal");queueWrite(sessionId,`${detail.command}\r`);};const handler=(event:Event)=>{const command=(event as CustomEvent<string|{type:string;command:string;commandId?:string;title?:string}>).detail;if(command==="new-terminal")void createTerminal("shell");else if(command==="run-tests")void runTests();else if(typeof command==="object"&&(command.type==="run-operational-command"||command.type==="run-terminal-command"))void runOperational(command);};window.addEventListener("control-command",handler);return()=>window.removeEventListener("control-command",handler);},[workspace?.projectId,workspace?.path,activeTerminalId]);

  if(!activeTerminal)return <div className="terminal-container empty"><p>No terminal active</p></div>;
  return <div className="terminal">
    <div className="terminal-tabs"><span className="terminal-title">Terminal</span>{Array.from(terminals.values()).map(term=><button key={term.id} className={`terminal-tab ${activeTerminalId===term.id?"active":""}`} onClick={()=>void selectTerminal(term.id)}><span>{term.name}</span><span className="terminal-tab-close" onClick={event=>{event.stopPropagation();void closeTerminal(term.id);}}>×</span></button>)}<button className="terminal-new-button" onClick={()=>void createTerminal("shell")} title="New terminal">+</button><span className="terminal-cwd" title={activeTerminal.cwd}>{activeTerminal.cwd}</span><button className="terminal-stop" onClick={()=>queueWrite(activeTerminal.id,"\u0003")}>Interrupt</button><button className="terminal-expand" title="Expand or restore terminal" onClick={()=>window.dispatchEvent(new CustomEvent("control-command",{detail:"toggle-terminal-size"}))}>↕</button></div>
    {error&&<div className="terminal-error"><span>{error}</span><button onClick={()=>setError(undefined)}>×</button></div>}
    <div className="terminal-xterm" ref={hostRef} onMouseDown={()=>xtermRef.current?.focus()}/>
  </div>;
}
