...
import { createId, getControlDatabase, getControlDataTopology, migrateControlToLocal, migrateControlToServer, restartControlDatabase, LOCAL_PARTICIPANT_ID, type ControlBackup, type ControlActivity, type ControlAgent, type ControlChangeSet, type ControlConversation, type ControlDecision, type ControlEvidence, type ControlInboxItem, type ControlInvestigation, type ControlMessage, type ControlMetadata, type ControlOutcome, type ControlParticipant, type ControlRun, type ControlRunEvent, type ControlTask, type ControlTestResult, type ControlWork } from "../lib/control-db";
...
export default function CoordinationPanel({ project,availableProjects=[project],initialTab="chat",hideNavigation=false,scope="project" }: {
    ...
    const [activeId, setActiveId] = useState<string>(), [selectedRunId, setSelectedRunId] = useState<string>(), [input, setInput] = useState(""), [sending, setSending] = useState(false),[startingConversation,setStartingConversation]=useState(false), [error, setError] = useState<string>(),[messageAction,setMessageAction]=useState<string>();
    ...
    const recoverData=async()=>{sessionStorage.setItem("control-chat-draft",input);await restartControlDatabase();};
    ...
    const visibleConversations=useMemo(()=>conversations.filter(item=>conversationScope==="global"?item.scope==="global":item.projectId===conversationScope&&item.scope!=="global"),[conversations,conversationScope]);
    const active = useMemo(() => visibleConversations.find(c => c.id === activeId), [visibleConversations, activeId]);
    useEffect(()=>setActiveId(id=>visibleConversations.some(item=>item.id===id)?id:visibleConversations[0]?.id),[conversationScope,visibleConversations]);
    useEffect(()=>setTab(initialTab),[initialTab]);
    const refresh = async () => { const version=++refreshVersion.current;const [a, t, r, runLog, changes,tests,events, c, d, w, people, inbox, i, e, o,metadata] = await Promise.all([db.agents.all(), db.tasks.find({ projectId: project.id }), db.runs.find({ projectId: project.id }), db.runEvents.find({ projectId: project.id }),db.changeSets.find({projectId:project.id}),db.testResults.find({projectId:project.id}), db.activity.find({ projectId: project.id }), scope==="global"?db.conversations.all():db.conversations.find({ projectId: project.id }), db.decisions.find({ projectId: project.id }), scope==="global"?db.works.all():db.works.find({ projectId: project.id }), db.participants.all(), db.inboxItems.find({ projectId: project.id }), db.investigations.find({ projectId: project.id }), db.evidence.find({ projectId: project.id }), db.outcomes.find({ projectId: project.id }),db.metadata.get("database")]);if(version!==refreshVersion.current)return; setAgents(a); setParticipants(people); setTasks(t); const orderedRuns=r.sort((x, y) => y.startedAt - x.startedAt); setRuns(orderedRuns); setRunEvents(runLog.sort((x,y)=>x.createdAt-y.createdAt));setChangeSets(changes);setTestResults(tests);setDatabaseMetadata(metadata||undefined); setSelectedRunId(id=>orderedRuns.some(item=>item.id===id)?id:orderedRuns[0]?.id); setActivity(events.sort((x, y) => y.createdAt - x.createdAt)); setConversations(c.sort((x, y) => y.updatedAt - x.updatedAt)); setDecisions(d.sort((x, y) => y.createdAt - x.createdAt)); setWorks(w); setInboxItems(inbox.sort((x, y) => y.createdAt - x.createdAt)); setInvestigations(i.sort((x, y) => y.updatedAt - x.updatedAt)); setEvidence(e.sort((x, y) => y.createdAt - x.createdAt)); setOutcomes(o.sort((x, y) => y.createdAt - x.createdAt)); setActiveId(id => c.some(item => item.id === id) ? id : undefined); };
    ...
    const createConversation = async (requestedTitle?:string):Promise<ControlConversation|undefined> => {
        ...
        if (!title)
            return;
        if(requestedTitle){
            const existing=conversations.find(item=>item.title===title&&!item.taskId&&(conversationScope==="global"?item.scope==="global":item.projectId===conversationScope&&item.scope!=="global"));
        ...
        if(!context)await bounded(db.works.insert({id:workId,projectId:targetProject.id,title,intent:"Conversation with Control Intelligence",kind:"investigation",status:"active",createdAt:now,updatedAt:now},workId),"Creating conversation context");
        ...
        await bounded(db.conversations.insert(conversation, id),"Saving conversation");
    ...
    const sendMessage = async (rawContent:string,retryMessage?:ControlMessage,conversationOverride?:ControlConversation) => {
        ...
        try {
            ...
            const activityId=createId("activity");await db.activity.insert({id:activityId,projectId:conversation.projectId,workId:conversation.workId,taskId:conversation.taskId,runId,type:"conversation_started",actor:agent.id,summary:`${agent.name} started a response in ${conversation.title}`,createdAt:now},activityId);
        }
        catch (reason) {
    ...
    const send=async()=>{const content=input.trim();if(!content||sending||startingConversation||firstSendInFlight.current)return;const projectIntent=parseCreateProjectIntent(content);if(projectIntent&&conversationScope==="global"){if(window.confirm(`Create a new Git project named ${projectIntent.name} in ${projectIntent.location[0].toUpperCase()+projectIntent.location.slice(1)} and open it in Control?`)){setInput("");window.dispatchEvent(new CustomEvent("control-create-project",{detail:projectIntent}));}return;}firstSendInFlight.current=true;setError(undefined);try{let conversation=active;if(!conversation){setStartingConversation(true);conversation=await createConversation(conversationScope==="global"?"Global":"General");}if(!conversation)throw new Error("Control could not create a conversation for this message");setStartingConversation(false);await sendMessage(content,undefined,conversation);}catch(reason){setError(reason instanceof Error?reason.message:String(reason));}finally{setStartingConversation(false);firstSendInFlight.current=false;}};
    const messageToTask=async(message:ControlMessage)=>{if(!active)return;const targetProject=availableProjects.find(item=>item.id===active.projectId)||project,now=Date.now(),workId=createId("work"),taskId=createId("task");const firstLine=message.content.split("\n").map(line=>line.replace(/^\s*[-#*>\d.)]+\s*/,"").trim()).find(Boolean)||"Follow up on conversation";const title=safe(firstLine).slice(0,100),description=safe(`${message.content}\n\nSource: ${active.title} conversation (${active.id}), response ${message.id}`),projectTasks=await db.tasks.find({projectId:targetProject.id}),order=projectTasks.reduce((highest,item)=>Math.max(highest,item.order??item.createdAt),0)+1;await db.works.insert({id:workId,projectId:targetProject.id,title,intent:description,kind:"task",status:"captured",createdAt:now,updatedAt:now},workId);await db.tasks.insert({id:taskId,workId,projectId:targetProject.id,title,description,status:"not-started",priority:"medium",urgency:"normal",tags:["from-chat"],blockedBy:[],acceptanceCriteria:[],feedback:[],modelPreference:"auto",order,createdAt:now,updatedAt:now},taskId);const activityId=createId("activity");await db.activity.insert({id:activityId,projectId:targetProject.id,workId,taskId,type:"task_created_from_conversation",actor:LOCAL_PARTICIPANT_ID,summary:`Task created from ${active.title}: ${title}`,details:`Conversation ${active.id}, message ${message.id}`,createdAt:now},activityId);setMessageAction(`Task created in ${targetProject.name}: ${title}`);window.setTimeout(()=>setMessageAction(undefined),5000);};
    const copyMessage=async(message:ControlMessage)=>{try{await navigator.clipboard.writeText(message.content);setMessageAction("Response copied");window.setTimeout(()=>setMessageAction(undefined),2500);}catch{setError("Could not copy this response");}};
    const followUpFrom=async(message:ControlMessage)=>{setInput(`Regarding this response:\n\n${message.content.slice(0,1200)}\n\n`);window.setTimeout(()=>document.querySelector<HTMLTextAreaElement>(".chat-input textarea")?.focus(),0);};
    ...
    return <div className="coordination-panel">
        ...
        {tab==="chat"&&<div className="chat-pane">
            ...
            <div className="conversation-toolbar"><select value={active?.id||""} onChange={event=>setActiveId(event.target.value||undefined)}><option value="">No conversation</option>{visibleConversations.map(item=><option key={item.id} value={item.id}>{item.title}{visibleConversations.filter(candidate=>candidate.title===item.title).length>1?` · ${new Date(item.createdAt).toLocaleString()}`:""}</option>)}</select><button title="New conversation" onClick={()=>setCreatingConversation(value=>!value)}>+</button></div>
            ...
            <div className="message-list">{messages.map(message=><div key={message.id} className={`message ${message.role} ${message.deliveryStatus||"delivered"}`}><span className="message-sender">{message.role==="user"?participants.find(item=>item.id===message.senderId)?.name||"You":agents.find(item=>item.id===message.senderId)?.name||message.senderId}{message.deliveryStatus==="pending"?" · sending":message.deliveryStatus==="failed"?" · failed":""}</span><div>{message.content}</div>{message.role==="assistant"&&message.deliveryStatus!=="failed"&&<div className="message-actions"><button onClick={()=>void messageToTask(message)}>Turn into task</button><button onClick={()=>void followUpFrom(message)}>Follow up</button><button onClick={()=>void copyMessage(message)}>Copy</button></div>}{message.deliveryStatus==="failed"&&<div className="message-failure"><span>{message.error}</span><button disabled={sending} onClick={()=>void sendMessage(message.content,message)}>Retry</button></div>}</div>)}{sending&&<div className="message-status">Control Intelligence is working…</div>}</div>
            <form className="chat-input" onSubmit={event=>{event.preventDefault();void send();}}><textarea value={input} onChange={event=>setInput(event.target.value)} onKeyDown={event=>{if(event.key==="Enter"&&!event.shiftKey){event.preventDefault();event.currentTarget.form?.requestSubmit();}}} placeholder={active?"Message Control Intelligence…":"Type a message to start a conversation…"} disabled={sending||startingConversation}/><button type="submit" disabled={!input.trim()||sending||startingConversation}>{startingConversation?"Starting conversation…":sending?"Sending…":"Send"}</button></form>
        </div>}
